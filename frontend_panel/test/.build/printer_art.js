 ...
 */
/* ------------------------------------------------------------------ pieces */
const SPOOL_CX = [66.5, 102, 137.5, 173];
const DEFAULT_SPOOLS = ['#d94a3d', '#2f7fd1', '#e8b33a', '#3aa87a'];
/**
 ...
 */
const chamberLight = (on, x, y, w) => svg `
  ${on
 ...
 */
const printedMass = (visible, progress, plateX, plateW, plateY, maxH, tip) => {
 ...
 */
const heatGlow = (heat, x, y, w, h) => heat <= 0.02
    ? nothing
 ...
 */
const coil = (cx, colour, remaining) => {
    const fill = colour !== null && colour !== void 0 ? colour : 'var(--ac-printer-rail, currentColor)';
    const frac = remaining === undefined ? 1 : Math.max(0, Math.min(1, remaining));
    const full = 32;
    const hub = 11;
 ...
 */
const resinPrint = (visible, progress, tip) => {
    if (!visible || progress <= 0.005)
        return nothing;
 ...
 * camera running.
 */
const screenFace = (status, cx, cy, r) => {
    if (status === 'paused') {
        const b = r * 0.32;
        return svg `<g>
...
const s1Body = ({ gantry, nozzle, tip = 'var(--ac-printer-accent, currentColor)', lightOn = false, progress = 0, cameraLive = false, nozzleHeat = 0, bedHeat = 0, fanOn = false, status = 'idle', }) => svg `
  <g fill="currentColor">
...
const aceModule = (spools, 
/** Slot to highlight, or -1 when this unit is not the one feeding. */
active = 0, feed = 'M188 88 C 216 94 214 116 196 122', 
  ...
  <g id="ace-${unit}">
    ...
    SPOOL_CX[active] === undefined
        ...
        : svg `<rect x="${SPOOL_CX[active] - 17}" y="36" width="34" height="44" rx="5" fill="none"
                stroke="${(_a = spools[active]) !== null && _a !== void 0 ? _a : 'currentColor'}" stroke-width="2"></rect>`}
...
const PRINTER_ART = {
    ...
    kobra_3: {
        ...
        chamber: [19, 24, 30, 24],
        travel: 92,
        body: ({ gantry, nozzle, tip = 'var(--ac-printer-accent, currentColor)', lightOn = false, progress = 0, cameraLive = false, nozzleHeat = 0, bedHeat = 0, fanOn = false, status = 'idle', }) => svg `
      <g fill="currentColor">
    ...
    resin: {
        ...
        body: ({ gantry, tip = 'var(--ac-printer-accent, currentColor)', progress = 0, cameraLive = false, status = 'idle', }) => svg `
...
function selectPrinterArt(machineName, 
/** How many ACE units the card reports attached: 0, 1 or 2. */
aceCount = 0, override, spools, activeSlot = 0, 
/** Per-slot remaining filament, 0-1, in the same order as `spools`. */
remaining = []) {
    const name = (machineName !== null && machineName !== void 0 ? machineName : '').toLowerCase();
    let kind = 'fdm';
    ...
    if (kind === 'resin')
        return art;
    return withFilamentSource(art, aceCount, spools, activeSlot, remaining);
}
/* ------------------------------------------------------- transform helpers */
/**
 ...
 */
const gantryTransform = (art, progress, cameraLive) => cameraLive
...
const nozzleTransform = (x, span = 48) => `translate(${(x * span).toFixed(1)} 0)`;
const ACE_PITCH = 101;
/**
...
function withAce(art, count, spools = DEFAULT_SPOOLS, active = 0, remaining = []) {
    ...
    return Object.assign(Object.assign({}, art), { kind: art.kind === 'kobra_s1' ? 'kobra_s1_combo' : art.kind, viewBox: `0 0 240 ${height}`, chamber: [
 ...
 */
const sideSpool = (color = 'var(--ac-printer-accent, currentColor)', cy = 70, 
...
const withFilamentSource = (art, aceCount, spools = DEFAULT_SPOOLS, active = 0, remaining = []) => aceCount > 0
    ? withAce(art, aceCount, spools, active, remaining)
    : Object.assign(Object.assign({}, art), { body: (st) => svg `
          ${sideSpool(st.tip, art.kind === 'kobra_3' ? 80 : 70, art.kind === 'fdm' ? 222 : 219.5)}
          ${art.body(st)}` });
/* -------------------------------------------------- filament colour lookup */
const NAMED_FILAMENT = {
...
function filamentColor(raw) {
    ...
    if (/^(rgb|hsl)a?\(/i.test(s))
        return s;
    return (_a = NAMED_FILAMENT[s.toLowerCase()]) !== null && _a !== void 0 ? _a : UNKNOWN;
}
/**
 ...
 */
const spoolColors = (slots = [], count = 1) => Array.from({ length: Math.max(1, count) * 4 }, (_, i) => {
    const slot = slots[i];
    if (!slot)
...
const rgbToHex = (rgb) => Array.isArray(rgb) && rgb.length >= 3
    ? '#' +
        rgb
            .slice(0, 3)
...
const activeTipColor = (slots, active = 0, singleFilament) => { var _a; return ((slots === null || slots === void 0 ? void 0 : slots.length) ? filamentColor((_a = slots[active]) === null || _a === void 0 ? void 0 : _a.color) : filamentColor(singleFilament)); };
/* ----------------------------------------------------------- render + CSS */
/**
 ...
 */
const renderPrinter = (art, state = {}) => {
    var _a, _b;
    return html `
  <svg class="ac-apr-svg" viewBox="${art.viewBox}" fill="none"
       preserveAspectRatio="xMidYMid meet" aria-hidden="true">
    ${art.body(Object.assign(Object.assign({}, state), { gantry: gantryTransform(art, ((_a = state.progress) !== null && _a !== void 0 ? _a : 0) * 100, !!state.cameraLive), nozzle: nozzleTransform((_b = state.nozzleX) !== null && _b !== void 0 ? _b : 0) }))}
  </svg>`;
};
/**
...
const cameraInset = (art) => art.chamber.map((v) => `${v}%`).join(' ');
/*
...
export { DEFAULT_SPOOLS, PRINTER_ART, SPOOL_CX, activeTipColor, anycubicCube, artAspect, cameraInset, filamentColor, gantryTransform, nozzleTransform, renderPrinter, rgbToHex, selectPrinterArt, sideSpool, spoolColors, withAce, withFilamentSource };
